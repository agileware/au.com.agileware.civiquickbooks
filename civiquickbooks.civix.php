...
spl_autoload_register('_civiquickbooks_civix_class_loader', TRUE, TRUE);

function _civiquickbooks_civix_class_loader($class) {
  if ($class === 'CRM_Civiquickbooks_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_Civiquickbooks_DAO_Base');
      // ^^ Materialize concrete names -- encourage IDE's to pick up on this association.
    }
    else {
      $realClass = 'CiviMix\\Schema\\Civiquickbooks\\DAO';
  ...
  if (strpos($class, 'CiviMix\\Schema\\Civiquickbooks\\') === 0) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _civiquickbooks_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _civiquickbooks_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _civiquickbooks_civix_fixNavigationMenu($nodes);
  }
}
...
function _civiquickbooks_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
